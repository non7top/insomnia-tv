...
class MockIrDriver : public IIrDriver {
public:
    void begin() override { began_ = true; }
    bool send(const String&, uint64_t, uint16_t) override { sendCount_++; return true; }
    void receive() override { receiving_ = true; }
    uint16_t* learn_raw(uint16_t& out_len) override { out_len = 0; return nullptr; }
...
class MockNetworkChecker : public INetworkChecker {
public:
...
public:
    ...
    bool uploadFile(const String&, const uint8_t*, size_t) override { return uploadResult_; }
    int32_t downloadFile(const String&, uint8_t*, size_t) override { return downloadResult_; }
...
public:
    bool connect(const String&, uint16_t, const String&, const String&, const String&) override
        { connected_ = true; return true; }
    void disconnect() override { connected_ = false; }
    bool isConnected() const override { return connected_; }
    int16_t publish(const String&, const String&, uint8_t, bool) override { publishCount_++; return 1; }
    int16_t subscribe(const String&, uint8_t) override { return 1; }
    void unsubscribe(const String&) override {}
    void setCallback(MqttCallback) override { hasCallback_ = true; }
...
void test_hal_polymorphism(void) {
    IIrDriver* ir = new MockIrDriver();
    ir->begin();
    delete ir;

    INetworkChecker* net = new MockNetworkChecker();
    TEST_ASSERT_TRUE(net->isConnected());
    delete net;

    IClock* clk = new MockClock();
    ...
    IMqttClient* mqtt = new MockMqttClient();
    TEST_ASSERT_FALSE(mqtt->isConnected());
    delete mqtt;
}

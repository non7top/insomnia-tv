#ifndef INSOMNIATV_IMqttClient_H
#define INSOMNIATV_IMqttClient_H
...
/** MQTT message callback signature: (topic, payload, length) */
using MqttCallback = std::function<void(const char* topic, const char* payload, size_t len)>;
...
public:
    ...
    /** Publish a message to a topic, returns message ID or -1 on failure */
    virtual int16_t publish(const String& topic, const String& payload,
                            uint8_t qos = 0, bool retain = false) = 0;

    /** Subscribe to a topic, returns subscription ID or -1 on failure */
...
#endif // INSOMNIATV_IMqttClient_H

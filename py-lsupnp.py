...
class lsupnp:
    ...
    def __str__(self):
        return (
                ...
                f"port={self.opt_port} "
                f"rdns={self.opt_rdns} "
                f"verbose={self.opt_verbose} "
                f"timeout={self.opt_timeout}"
                )
    ...
    def discover_hosts(self):
        ...
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_BINDTODEVICE, b'enp0s8')
        ...
        try:
            ...
            while True:
                ...
                if self.opt_verbose:
                    ...
                    print(f'{data.decode(sys.stdout.encoding)}')

        except socket.timeout:
            if self.opt_verbose:
                print(f"Socket timed out after {self.opt_timeout:.1f} seconds")
        finally:
            sock.close()
            ...
            for host in hosts:
                print(f"{host}\t", end='')
                if self.opt_rdns:
                    print(f"{self._rdns_lookup(str(host))}", end='')
                print("")
...
def main():
    ...
    parser.add_argument('-t', '--timeout', dest='opt_timeout', action='store', type=float, metavar='TIMEOUT',
            help=f'Specify socket timeout interval in seconds (default is {DEFAULT_TIMEOUT})')
